...
- Main package **`@byteink/mppjs`** — pure TS, MIT, OS-agnostic. (Earlier names — unscoped `mpxjs` and scoped `@byteink/mpxjs` — were both burned: the first by npm's similarity check vs `rxjs`, the second by a `npm unpublish` 24h cooldown after a v0.1.2 mishap.) Declares each `@byteink/mppjs-<platform>` as an `optionalDependency` with `os`/`cpu` constraints, so package managers install only the matching one.
- Per-platform sidecars under [packages/](packages/) — `@byteink/mppjs-{darwin-arm64,linux-arm64,linux-x64,win32-x64}`, each LGPL-2.1-or-later (because they ship a binary derived from MPXJ). Intel-mac (darwin-x64) was dropped because GitHub's free `macos-13` runner pool is unusable.
...
- Binary lookup chain in [src/platform.ts](src/platform.ts): `MPPJS_BINARY` env / option override → installed `@byteink/mppjs-<platform>` sidecar via `require.resolve` → local `bin/` (development).
...
2. Linux publish job downloads artifacts, syncs version across all `package.json` files (root + 4 sidecars), compiles TS, then `npm publish` each sidecar followed by the main package — all with `--access public --provenance`.
